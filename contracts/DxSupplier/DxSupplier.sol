...
contract DxSupplier is Ownable, IDxSupplier, ReentrancyGuard {
    ...
    constructor(uint256 _dxPerBlock, address _vault) public {
        ...
        if (_from <= lastWeekPoint && _to >= lastWeekPoint) {
            return
                lastWeekPoint.sub(_from).mul(SAFE_MULTIPLIER).add(
                    _to.sub(lastWeekPoint).mul(SAFE_MULTIPLIER).mul(min).div(max)
                );
        }
